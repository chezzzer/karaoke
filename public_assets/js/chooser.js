...
function queueItem(uri, isrc) {
    $(`[track-uri='${uri}'] .add-queue`).attr(
        "class",
        "fa-duotone fa-spinner-third fa-2x fa-spin pointer add-queue"
    );
        ...
        <div class="col-sm-${column} mb-4">
            <div class="bg-darker shadow d-flex p-2 rounded" track-uri="${item.uri}">
                <a href="https://open.spotify.com/track/${item.id}" target="_blank">
                    ...
                    }" width="48px" height="48px" style="object-fit:cover" class="shadow-sm rounded">
                </a>
                <div class="ms-3 my-auto text-nowrap overflow-hidden me-2">
                    <div class="fw-bold">${item.name}</div>
                    <div class="text-muted">${combineArtists(
                        item.artists
                    )} <span class="extra"></span>${
        ableToQueue
            ? ""
            : `<span class="admin-item admin-remove-queue text-decoration-underline pointer d-inline" onclick="removeQueue('${item.id}')">Remove</span>`
    }</div>
                </div>
...
function refreshCatalogue() {
    socket.send(JSON.stringify({ method: "catalogue" }));
}

$(document).on("ws/data", (event, data) => {
    if (data.return == "connection/success") {
        if (!$(".catalogue .col-sm-3").length) {
            refreshCatalogue();
        }
        ...
        if (data.uri) {
            ...
            $(`[track-uri='${data.uri}'] .add-queue`).attr(
                "class",
                "fa-solid fa-times fa-2x pointer add-queue"
            );
        }
    } else if (data.return == "karaoke/queued") {
        $(`[track-uri='${data.uri}'] .add-queue`).attr(
            "class",
            "fa-solid fa-check fa-2x pointer add-queue"
        );
        $(`[track-uri='${data.uri}']`).removeClass("adding-queue");
        $(`[track-uri='${data.uri}']`).addClass("added-queue");
    } else if (data.return == "spotify/catalogue") {
        $(".catalogue").html("");
        data.payload.forEach((item) => {
            $(".catalogue").append(makeSongItem(item));
        });
        ...
        if (!queue.length) {
            $(".queue").html(
                '<div class="col-sm-12"><div class="text-muted p-2 mx-auto shadow bg-darker rounded">No songs queued, search for a song to add to the list!</div></div>'
            );
        }
        queue.forEach((item) => {
        ...
        if (!$(".admin-logout").length) {
            $("#footer").append(
                `<a onclick="setCookie('admin_password', '');location.reload()" class="text-muted text-decoration-none ps-1 pointer admin-logout">Log Out</a>`
            );
        }
...
setInterval(() => {
    if (!current.paused) {
        ...
        $("#song-progress .progress-bar").css(
            "width",
            (current.time / current.duration) * 100 + "%"
        );
    }
    let time = new Date();
    $("#time").html(
