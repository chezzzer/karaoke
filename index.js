...
${playback.item.artists[0].name} - ${playback.item.name}
                `);
                    ...
                    let palette = await Vibrant.from(
                        current.playback.item.album.images[2].url
                    ).getPalette();
                    ...
                    progress + 1500 < playback.progress_ms ||
                    progress - 1500 > playback.progress_ms
                ) {
...
function getLyrics(track) {
    ...
    return new Promise((resolve) => {
        ...
        let token =
            config.musixmatch_usertokens[
                Math.floor(Math.random() * config.musixmatch_usertokens.length)
            ];

        //formulate URL to musixmatch API.
        let lyricURL = `https://apic-desktop.musixmatch.com/ws/1.1/macro.subtitles.get?format=json&q_track=${
            track.name
        }&q_artist=${track.artists[0].name}&q_artists=${artists}&q_album=${
            track.album.name
        }&user_language=en&f_subtitle_length=${(track.duration_ms / 1000).toFixed(0)}&q_duration=${
            track.duration_ms / 1000
        }&tags=nowplaying&namespace=lyrics_synched&track_spotify_id=spotify:track:${
            track.id
        }&f_subtitle_length_max_deviation=1&subtitle_format=mxm&app_id=web-desktop-app-v1.0&usertoken=${token}`;
...
app.ws("/", async (ws, req) => {
    ...
    ws.on("message", (msg) => {
        ...
        if (data.method == "search") {
            if (data.value) {
                spotify.search(data.value, ["track"], { limit: 12 }).then((data) => {
                    ws.send(
                        JSON.stringify({
            ...
            if (data.isrc && data.uri) {
                ...
                axios
                    .get(
                        `https://api.musixmatch.com/ws/1.1/track.get?track_isrc=${data.isrc}&apikey=86141b48c8ba2f0bce3feb4a5f728a59`
                    )
                        ...
                        if (
                            ...
                            return ws.send(
                                JSON.stringify({
                                    ...
                                    payload:
                                        "That track isn't available on our library or may not have lyrics.",
                                    uri: data.uri,
                                })
                            );
...
app.listen(config.port, () => {
    ...
    Object.keys(os.networkInterfaces()).forEach((iface) => {
        iface = os.networkInterfaces()[iface];
        ip = iface.find((i) => {
